...
permalink: /compare/
description: <h1> Me vs. The Machine </h1> <br> The image juxtaposes my photography againt one generated by GPT-4 using a caption that GPT-4 itself generated, when prompted with my photography. <br> Click/Tap anywhere on the image to change the percentage of how much of each image is shown. <br> Click on the \Random\ button to generate a random image, or cycle thru them all sequentyally (Current count - 25 Pairs)
nav: true
nav_order: 8
---
<style>
...
.arrow-button {
    ...
    cursor: pointer;
    padding: 10px;
    background-color: rgba(55, 55, 55, 0.5); /* Adjust color/transparency as desired */
    color: white;
    border: none;
    border-radius: 50%;
    width: 40px;
